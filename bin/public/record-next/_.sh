...
record-next () {
  ...
  if [[ -z "$@" ]]; then
    if lynx --dump "http://whatismyipaddress.com/" | grep "Comcast Cable" ; then
      ...
      local +x PATTERN="$COMMON|#TOKYO|Kuala|Somewhere|Rising|Technology|Supernova|Cherry|Amazing Animals|Biz Lab|Insight|HOKUSAI|SOMEWHERE|COOL JAPAN|Doki|Document|Documentary|Buzz|Focus|Melo|Close-up|Inside"
    fi
    # case "$(date +"%I:%M")" in
